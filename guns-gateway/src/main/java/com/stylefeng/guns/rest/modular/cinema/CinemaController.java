...
public class CinemaController {
    ...
    public ResponseVO getCinemas(CinemaQueryVO cinemaQueryVO){
        ...
        try{
            //按照五个条件进行筛选
            Page<CinemaVO> cinemas = cinemaServiceApi.getCinema(cinemaQueryVO);
    ...
    //获取影院的查询条件
    /*
        1、热点数据 -> 放缓存(更好的放 Redis Memcache)
        2、banner无条件且经常被访问 -> 放缓存
        Redis和Memcache属于分布式缓存，Dubbo自带的缓存更类似于EsCache和OsCache本地缓存，相对于分布式缓存速度快，但是会存储多份。
     */
    ...
    public ResponseVO getFields(Integer cinemaId){
        ...
        try{
            ...
            return ResponseVO.success("获取播放场次失败");
        }
    }
    ...
    public ResponseVO getFieldInfo(Integer cinemaId, Integer fieldId){
        ...
        try{
            ...
            return ResponseVO.success("获取选座信息失败");
        }
    }
